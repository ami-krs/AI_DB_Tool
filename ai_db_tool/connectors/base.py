...
class DatabaseManager:
    ...
    def _build_connection_string(self, config: DatabaseConfig) -> str:
        ...
        if db_type == 'sqlite':
            # SQLAlchemy SQLite: sqlite:///path (works for both relative and absolute)
            # For absolute paths: sqlite:////absolute/path (4 slashes) or sqlite:///absolute/path (3 slashes)
            # SQLAlchemy handles this automatically - sqlite:/// + /absolute/path = sqlite:////absolute/path
            # Ensure we use absolute path for consistency
            db_path = str(Path(config.database).absolute())
