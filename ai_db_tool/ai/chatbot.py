    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['INSERT', 'POPULATE', 'ADD RECORDS', 'CREATE RECORDS', 'ADD DATA', 'TEST RECORDS']):
                ...
                prompt += "   b. For EACH table, locate its EXACT column list in the schema (format: 'Table: table_name\\n  Columns: col1 (type), col2 (type), ...')\n"
                prompt += "   c. Copy the EXACT column names from the schema - use them exactly as shown\n"
                prompt += "   d. Use ONLY those exact column names - DO NOT modify, guess, or invent column names\n"
                prompt += "4. CRITICAL: You MUST use ONLY column names that actually exist in the schema above\n"
                prompt += "   - Look at the schema format: 'Table: procurement\\n  Columns: procurement_id (INTEGER), purchase_order_id (INTEGER), item_name (VARCHAR)'\n"
                prompt += "   - Use ONLY the column names shown: 'procurement_id', 'purchase_order_id', 'item_name'\n"
                prompt += "   - DO NOT use 'status', 'procurement_date', 'item_id' if they are NOT in that column list\n"
                prompt += "   - DO NOT assume common column names exist - ONLY use what is explicitly listed\n"
                prompt += "5. If the schema shows 'Table: procurement\\n  Columns: procurement_id (INTEGER), purchase_order_id (INTEGER)'\n"
                prompt += "   Then you MUST use ONLY: `pr.procurement_id, pr.purchase_order_id`\n"
                prompt += "   DO NOT add 'status', 'date', or any other columns not in the list\n"
                prompt += "6. If you cannot see the column list for a table in the schema, use ONLY the join column and primary key\n"
