    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['UPDATE', 'MODIFY', 'CHANGE', 'SET', 'EDIT']):
                ...
                prompt += "2. Find that table's column list in the schema above\n"
                ...
                prompt += "   - Look at the department table's columns in the schema\n"
                prompt += "   - Use the EXACT column name that appears there (might be 'department_id', 'id', etc.)\n"
                prompt += "   - Example: If schema shows 'department_id INTEGER', use 'department_id'\n"
                prompt += "   - Example: If schema shows 'id INTEGER', use 'id'\n"
