    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['INSERT', 'POPULATE', 'ADD RECORDS', 'CREATE RECORDS', 'ADD DATA', 'TEST RECORDS']):
                ...
                prompt += "      - For foreign key columns: CRITICAL - You MUST check what values actually exist in the referenced table\n"
                prompt += "        * If the schema shows foreign key relationships (e.g., 'department_id' references 'department' table):\n"
                prompt += "          - First, generate a SELECT query to check existing values: SELECT department_id FROM department;\n"
                prompt += "          - OR generate INSERT statements for the parent table first if it's empty\n"
                prompt += "          - OR use NULL if the foreign key column allows NULL values\n"
                prompt += "        * NEVER guess foreign key values - they must exist in the referenced table or the INSERT will fail\n"
                prompt += "        * If you cannot determine existing values, generate INSERT statements for the parent table first\n"
