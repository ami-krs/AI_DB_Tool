    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['INSERT', 'POPULATE', 'ADD RECORDS', 'CREATE RECORDS', 'ADD DATA', 'TEST RECORDS']):
                ...
                prompt += "3. CRITICAL: You MUST use ONLY column names that actually exist in the schema above\n"
                prompt += "   - Look at the schema above - find the table names (e.g., 'purchase_order', 'procurement')\n"
                prompt += "   - For EACH table, find its column list in the schema\n"
                prompt += "   - Use ONLY the column names that are listed in the schema for that specific table\n"
                prompt += "   - NEVER invent or guess column names like 'procurement_date', 'item_id', 'quantity' unless they are in the schema\n"
                prompt += "   - Example: If schema shows 'procurement' table has columns: 'procurement_id', 'purchase_order_id', 'status'\n"
                prompt += "     Then use ONLY those columns: `SELECT pr.procurement_id, pr.purchase_order_id, pr.status`\n"
                prompt += "     DO NOT use columns like 'procurement_date', 'item_id' if they are NOT in the schema\n"
                prompt += "4. Instead, explicitly list ALL columns with table aliases using ONLY schema-defined columns:\n"
                prompt += "   - Example: `CREATE VIEW my_view AS SELECT t1.col1, t1.col2, t2.col3, t2.col4 FROM table1 t1 JOIN table2 t2 ON t1.id = t2.id`\n"
                prompt += "   - But ONLY if col1, col2, col3, col4 actually exist in the schema for those tables\n"
                prompt += "5. If a join column exists in both tables, select it only ONCE from one table\n"
                prompt += "6. Use table aliases (e.g., `po`, `pr`) and prefix all column names with the alias\n"
                prompt += "7. List columns explicitly: `SELECT po.column1, po.column2, pr.column3, pr.column4`\n"
                prompt += "   - But ONLY use column1, column2, column3, column4 if they are ACTUALLY in the schema\n"
                prompt += "8. DO NOT use `SELECT po.*, pr.*` - this will cause duplicate column errors\n"
                prompt += "9. DO NOT invent column names - check the schema first, then use only those columns\n"
                prompt += "10. Generate the CREATE VIEW statement with explicit column list using ONLY schema-defined columns\n"
                prompt += "=== END CRITICAL INSTRUCTIONS ===\n"
