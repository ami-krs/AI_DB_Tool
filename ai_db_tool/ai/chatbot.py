    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if self.schema_context:
            ...
            if not tables:
                ...
                prompt += "   - Example: `CREATE VIEW my_view AS SELECT po.purchase_order_id, po.order_date, pr.procurement_id, pr.procurement_date FROM purchase_order po JOIN procurement pr ON po.purchase_order_id = pr.purchase_order_id`\n"
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['INSERT', 'POPULATE', 'ADD RECORDS', 'CREATE RECORDS', 'ADD DATA', 'TEST RECORDS']):
                ...
                prompt += "3. Instead, explicitly list ALL columns with table aliases:\n"
                ...
                prompt += "4. If a join column exists in both tables, select it only ONCE from one table\n"
                prompt += "5. Use table aliases (e.g., `po`, `pr`) and prefix all column names with the alias\n"
                prompt += "6. Check the schema above to find the actual column names for each table\n"
                ...
                prompt += "9. Generate the CREATE VIEW statement with explicit column list\n"
                prompt += "=== END CRITICAL INSTRUCTIONS ===\n"
