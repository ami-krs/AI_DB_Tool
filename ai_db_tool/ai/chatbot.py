    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if self.schema_context:
            ...
            if not tables:
                ...
                prompt += "- Refuse UPDATE/DELETE/INSERT queries if the table name appears in the schema above\n"
                prompt += "\n"
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['INSERT', 'POPULATE', 'ADD RECORDS', 'CREATE RECORDS', 'ADD DATA', 'TEST RECORDS']):
                ...
                prompt += "3. ABSOLUTE REQUIREMENT: Before generating SQL, you MUST:\n"
                prompt += "   a. Find the table names in the schema above (e.g., 'purchase_order', 'procurement')\n"
                prompt += "   b. For EACH table, locate its EXACT column list in the schema\n"
                prompt += "   c. Write down ONLY the columns that are ACTUALLY listed for each table\n"
                prompt += "   d. Use ONLY those exact column names - nothing else\n"
                prompt += "4. CRITICAL: You MUST use ONLY column names that actually exist in the schema above\n"
                prompt += "   - If the schema shows 'procurement' table has columns: 'procurement_id', 'purchase_order_id', 'item_name'\n"
                prompt += "     Then you MUST use ONLY: `pr.procurement_id, pr.purchase_order_id, pr.item_name`\n"
                prompt += "   - DO NOT use 'status', 'procurement_date', 'item_id' if they are NOT in the schema\n"
                prompt += "   - DO NOT assume common column names exist - check the schema first\n"
                prompt += "5. If you cannot see the column list for a table in the schema, DO NOT guess - use only what you can see\n"
                prompt += "6. Explicitly list ALL columns with table aliases using ONLY schema-defined columns:\n"
                ...
                prompt += "7. If a join column exists in both tables, select it only ONCE from one table\n"
                prompt += "8. Use table aliases (e.g., `po`, `pr`) and prefix all column names with the alias\n"
                prompt += "9. List columns explicitly: `SELECT po.column1, po.column2, pr.column3`\n"
                prompt += "   - But ONLY use column1, column2, column3 if they are ACTUALLY in the schema\n"
                prompt += "10. DO NOT use `SELECT po.*, pr.*` - this will cause duplicate column errors\n"
                prompt += "11. DO NOT invent column names - if a column is not in the schema, DO NOT use it\n"
                prompt += "12. Generate the CREATE VIEW statement with explicit column list using ONLY schema-defined columns\n"
                prompt += "13. If the schema doesn't show column details, use a minimal set: just the join column and one other column from each table\n"
                prompt += "=== END CRITICAL INSTRUCTIONS ===\n"
