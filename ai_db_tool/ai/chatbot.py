    ...
    def _build_prompt(self, user_message: str, include_sql: bool) -> str:
        ...
        if include_sql:
            ...
            if any(keyword in user_upper for keyword in ['UPDATE', 'MODIFY', 'CHANGE', 'SET', 'EDIT']):
                ...
                prompt += "6. If the user says 'department id = department id + 1000', find the actual column name:\n"
                prompt += "   - Look at the department table's columns in the schema above\n"
                prompt += "   - Use the EXACT column name that appears there (might be 'department_id', 'id', etc.)\n"
                prompt += "   - Example: If schema shows 'department_id INTEGER PRIMARY KEY', use 'UPDATE department SET department_id = department_id + 1000'\n"
                prompt += "   - Example: If schema shows 'id INTEGER PRIMARY KEY', use 'UPDATE department SET id = id + 1000'\n"
                prompt += "   - WRONG: If schema shows 'department_id' but you use 'id' - this will cause an error!\n"
                prompt += "7. Generate the UPDATE statement using the EXACT column names from the schema\n"
