...
def main():
    ...
    if st.session_state.active_section not in ['home', 'smart_email_agent']:
        ...
        st.markdown("<div style='margin-top:-3.0rem; padding:0;'></div>", unsafe_allow_html=True)
        # Home icon + page label inline on the same row
        home_col1, home_col2 = st.columns([1, 20], vertical_alignment="center")
        with home_col1:
        ...
        if not (st.session_state.layout_mode == 'three_column' and st.session_state.connected):
